...
1. 管理画面でページを作成（「○○が△△と共演した作品」）
2. 出演を確認した作品の **タイトルをカンマ区切りで入力** して一括検索
   - 全文一致→一意なら自動選択 / 複数なら選ぶ / 0件は要再検索
   - 全文0件時は自動で短縮して再検索(短縮ヒットは取り違え注意の警告付き)
3. 正しい作品を選んで取得 → D1に保存(この時点で非公開・未チェック)
4. 各作品の **紹介文をリライト**(公式文の丸写し禁止・事実を変えない)し、確認して承認
